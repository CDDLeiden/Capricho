...
from CompoundMapper.chembl import filtered_data_workflow, molecule_info_from_chembl
...
for key, values in jump_SW_chembl_ids.items():
    ...
    try:
        chembl_data = filtered_data_workflow(values, confidence_scores=[6, 7, 8, 9])
        ...
        if full_df.shape[0] != chembl_data.shape[0]:
            logger.warning(f"Merging went wrong??? check {key} - {unique_mol_chembl_ids}")
        if full_df["molecule_chembl_id"].nunique() != chembl_data["molecule_chembl_id"].nunique():
            logger.warning(f"Fetching the data went wrong??? check {key} - {unique_mol_chembl_ids}")
